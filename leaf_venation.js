...
const BIRTH_DISTANCE = 20 * 20;
const AUXIN_RADIUS = 10;
const VEIN_RADIUS = 2;
const COLONIE_LIFESPAN = 500; // ms
const VEIN_WIDTH = 3;
...
let colonies = [];
// colonies = [{ auxins: [], root: new Vein(300, 300, VEIN_COLOR), veins: [], birth: performance.now() }]
// colonies[0].veins.push(colonies[0].root)
//let auxins = [];
...
function removeCloseAuxins(colonie) {
  ...
  auxins.forEach((a, i) => {
    veins.forEach((v) => {
      let distance = (a.x - v.x) * (a.x - v.x) + (a.y - v.y) * (a.y - v.y);
...
function setGrowDirections(colonie) {
  ...
  auxins.forEach((a, i) => {
    target = veins[0];
    targetDistance = (a.x - veins[0].x) * (a.x - veins[0].x) + (a.y - veins[0].y) * (a.y - veins[0].y);
    veins.forEach((v) => {
      if ((distance = (a.x - v.x) * (a.x - v.x) + (a.y - v.y) * (a.y - v.y)) < targetDistance) {
        target = v;
  ...
  veins.forEach((v) => {
    v.direction = { x: 0, y: 0 };
    if (w = v.watchers.pop()) minDistance = (w.x - v.x) * (w.x - v.x) + (w.y - v.y) * (w.y - v.y);
    v.watchers.forEach((w) => {
      let distance = (w.x - v.x) * (w.x - v.x) + (w.y - v.y) * (w.y - v.y);

      if (distance < minDistance) {
        v.direction = { x: (w.x - v.x), y: (w.y - v.y) };
        minDistance = distance;
      }
    })
    v.direction.x = v.direction.x / (Math.sqrt(minDistance));
    v.direction.y = v.direction.y / (Math.sqrt(minDistance));
  })
}
function growNewVeins(colonie) {
  ...
  veins.forEach((v) => {
    ...
    if (v.direction.x != 0 || v.direction.y != 0) {
      let newVein = new Vein(v.x + v.direction.x * VEIN_RADIUS, v.y + v.direction.y * VEIN_RADIUS, VEIN_COLOR);
      newVein.parent = v;
      colonie.veins.push(newVein);
    }
  })
}
...
function drawFrame() {
  ...
  if (time - lastBirth > COOLDOWN) {
    let newColonie = { auxins: [], root: new Vein(mouse.x, mouse.y, VEIN_COLOR), veins: [], birth: time };
    newColonie.veins.push(newColonie.root)
  ...
  colonies.forEach((c, i) => {
    ...
    if (age > COLONIE_LIFESPAN) age = COLONIE_LIFESPAN;
    const alpha = Math.floor((1 - (age / COLONIE_LIFESPAN)) * 255);
    ctx.strokeStyle = VEIN_COLOR + alpha.toString(16).toUpperCase();
    // console.log(alpha.toString(16).toUpperCase());
    ctx.lineWidth = VEIN_WIDTH;
    ctx.beginPath()
    c.veins.forEach((v) => {
      if (v.parent != null) {
        ctx.moveTo(v.x, v.y);
        ctx.lineTo(v.parent.x, v.parent.y);
      }
    })
    ctx.stroke();
  })

  const newColonies = colonies.filter((_, index) => !(index in dead));
